...
def _calculate_algbw(evt: Dict[str, Any]) -> float:
    ...
    # However, it uses 1e9 to convert BW from B/s to GB/s
...
def calculate_bw_(trace_data):
    nccl_events = [i for i in trace_data['traceEvents']
...
def calculate_sbw(trace_data):
    ...
    time_range_tree = IntervalTree([Interval(evt['ts'], evt['ts'] + evt['dur'])
                                    for evt in nccl_events])
...
def pick_iter_e2e_time_(trace_data, tl):
    tl.extend([
        evt['dur'] for evt in trace_data['traceEvents']
        if evt.get('cat', '') == 'user_annotation' and evt['name'].startswith('ProfilerStep#')
    ])
...
def analyze_profiler_trace(trace_dir: str, report_dir: str):
    ...
    Args:
        trace_dir (str): Directory path of input traces, where the trace name
                         should be in "rank-n.json" format.
    ...
    # list of iteration times (all ranks)
    iter_e2e_time = []

    # list of shared bw (all ranks)
    sbw_lst = []

    # key: (kernel_name, data size, ranks number)
    # value: list of [dur, algbw, busbw, pg]
    ...
    with open(os.path.join(report_dir, 'profiler_trace_summary_report.txt'), 'w', encoding='utf-8') as f:
        f.write(
            f'avg. E2ETime of iters among all ranks: '
            f'{sum(iter_e2e_time) / len(iter_e2e_time) / 1e3 :.3f} ms\n'
        )
        f.write(
            f'avg. SharedBW '
            f'(sum(data_size * busbw_factor) / GPU_comm_busy_time per rank) '
        ...
        for k, v in comm_bw_summary.items():
            f.write(
                f'{k[0]:>50s} {k[1]:>12d} {k[2]:>6d}|{v[0]:>5d}|{v[1]/1e3:>10.3f} '
            )
...
def main():
    """
    Entry point for analyzing PyTorch profiler traces via command line.
    """
    parser = argparse.ArgumentParser(
        description="Analyze PyTorch profiler traces and generate reports."
    )
    parser.add_argument(
        "--trace-dir",
        required=True,
        help="Directory containing profiler trace files (e.g. rank-N.json)."
    )
    parser.add_argument(
        "--report-dir",
        required=True,
        help="Directory where reports and processed traces will be written."
    )
