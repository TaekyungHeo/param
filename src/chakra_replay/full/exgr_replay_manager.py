...
class ExgrReplayManager:
    def __init__(self):
        ...
        try:
            ...
            self.parallel_nodes_parents = add_internal_parallel_nodes_parents(self.parallel_nodes_parents)
        ...
        self.tensor_allocate_mode: TensorAllcationMode = TensorAllcationMode.PRE_ALLOCATE
    ...
    def initBench(self):
        ...
        # Multiple traces.
            ...
            logging.info(f"process {os.getpid()} is rank {self.comms_env_params['global_rank']}")
            ...
            if "://" in self.args.trace_path:
                try:
                    ...
                    skip_node_json = read_remote_skip_node_file(f"{self.args.trace_path}")
                    if skip_node_json:
        ...
        self.resource_dir = os.path.join(base_path, os.path.splitext(file_name)[-2] + "_resources")
        ...
        if self.args.skip_node_file:
            try:
                ...
                logging.info(f"Failed to load skip node file {self.args.skip_node_file}.")
        ...
        logging.info(f"Available memory: {self.available_memory} GB")
        ...
        if self.tensor_with_device:
            self.tensor_registry = {
                k: (
                    None
                    if v is None
                    else (v.cpu() if self.tensor_device[k] == "cpu" or self.cpu else v.cuda(self.device))
                )
                for k, v in self.tensor_registry_permanent.items()
            }
        else:
            self.tensor_registry = {
                k: (None if v is None else (v.cpu() if k in self.cpu_tensor or self.cpu else v.cuda(self.device)))
                for k, v in self.tensor_registry_permanent.items()
            }
        gc.collect()
    ...
    def extract_subgraph(self, root):
        ...
        if len(self.profile_step_node_ids) > 1:
            ...
            self.sorted_nodes = [x for x in self.sorted_nodes if x.id > start_id and x.id < end_id]

        logging.info(f"#Operators to execute: {len(self.sorted_nodes)}")
        picked_nodes = []
    ...
    def analyze_tensors(self):
            ...
            for _, t_id, shape in get_input_tensors(node):
                if self.tensor_with_device:
                    ...
                    if t_id in self.input_tensor_ids:
                        add_unique_tensor(node.name, node.id, t_id, shape, input=True, device=device)
                else:
                    if t_id in self.input_tensor_ids:
        ...
        for node in self.sorted_nodes:
            ...
            for _, t_id, _ in get_input_tensors(node):
                if self.tensor_with_device:
                    t_id = tuple(list(t_id)[:5])
                if t_id in self.input_tensor_ids and self.tensors_mapping[(node.id, t_id, True)] not in output_set:
    ...
    def allocate_tensors(self):
        ...
        logging.info(f"Tensor allocation time: {(time.time_ns() - start_ns) / 1000000.0} ms")
    ...
    def allocate_node_tensors(self, node, is_input, is_comm_node):  # noqa: C901
        ...
        for idx, (data_type, t_id, shape) in enumerate(tensors):
            ...
            if not found_tensor:
                try:
                    dtype, _ = TORCH_DTYPES_RNG[data_type.removeprefix("Tensor(").rstrip(")")]
                    ...
                    if (
                        ...
                        tensor = self.get_tensor_from_file(node.id, idx, device, shape, dtype, strides)
                    ...
                    if data_type != "Tensor(nullptr (uninitialized))":
                        logging.info(f"KeyError: {node.id}, {t_id}, {data_type}")
                    if self.tensor_allocate_mode == TensorAllcationMode.PRE_ALLOCATE:
    ...
    def build_func(self, node):
        if node.kernel_backend == "triton":
            if node.kernel_file in self.kernel_map:
                ...
                func, output_count = build_triton_func(node, self.resource_dir, self.async_compile, self.device)
    ...
    def generate_code(self):
                    ...
                    if (
                        ...
                        try:
                            if is_fbgemm_forward(node):
                                ...
                                tensor_allocation_str += f"tensor_{replay_t_id} = input_args[{idx}]\n"
                                ...
                                if dtype == "Tensor(signed char)":
                                    dtype_str, rng_str = TORCH_DTYPES_RNG_str["signed char"]
                                else:
                                    dtype_str, rng_str = TORCH_DTYPES_RNG_str[dtype.removeprefix("Tensor(").rstrip(")")]
                            ...
                            if dtype != "Tensor(nullptr (uninitialized))":
                                logging.info(f"KeyError: {node.id}, {t_id}, {dtype}")
                            tensor_allocation_str += f"global tensor_{replay_t_id}\n"
                        ...
                        if replay_t_id not in unallocated_tensors:
                            tensor_allocation_str += f"global tensor_{replay_t_id}\n"
                            unallocated_tensor_allocation_str += f"    global tensor_{replay_t_id}\n"
        ...
        def _generate_inputs_str(node):
            ...
            if is_fbgemm_forward(node):
                ...
                for idx, item in enumerate(node.inputs):
                    if node.name == "aten::convolution_backward" and idx == len(node.inputs) - 1:
                    ...
                    if is_tensor(node, idx, True):
                        ...
                        elif (node.name == "aten::index_add_" and idx == 2) or (
                            node.name == "aten::index_select" and idx == 2
                        ):
                            inputs += f"tensor_{self.tensors_mapping[(node.id, tuple(item), True)]}.to(torch.int), "
                        elif (
                            node.name == "aten::index_copy_" and idx == 3 and node.input_types[idx] == "Tensor(double)"
                        ):
                            inputs += f"tensor_{self.tensors_mapping[(node.id, tuple(item), True)]}.to(torch.float64), "
                        elif node.name == "aten::index_copy_" and idx == 2 and node.input_types[idx] == "Tensor(long)":
        ...
        def _generate_outputs_str(node, override=True):
            ...
            def _parse_element_type(node, output_type, output_tensors, override):
                ...
                if output_type.startswith("Tensor"):
                    outputs += _generate_output_tensor_str(node, output_tensors, override) + ", "
                    ...
                    for element_type in elements_type:
                        outputs += _parse_element_type(node, element_type, output_tensors, override)
                    outputs = outputs[:-2] + "], "
                else:
                    outputs += "_, "
                return outputs
            ...
            try:
                ...
                for output_type in node.output_types:
                    outputs += _parse_element_type(node, output_type, output_tensors, override)

                assert len(output_tensors) == 0
                outputs = outputs[:-2]
                return outputs
            except Exception as e:
                logging.info(f"Generate outputs error: {e}, {node.id}")
                exit(1)
            ...
            for node in self.sorted_nodes:
                if node.name == "record_param_comms" and not self.compute_only:
                    if node.id in self.parallel_nodes_ids:
                        ...
                        code_str += f"            _ = traceBench.replaySingle(comms_params, {node.id})\n"
                ...
                if node.id in self.parallel_nodes_ids:
                    ...
                    start_special = False
                    code_str += f"        # node id: {node.id}\n"
                    code_str += exec_template.format(outputs=outputs_str, func=func_str, inputs=inputs_str) + "\n"
        ...
        code_str += generate_suffix(self.numWarmupIters, self.numIters, self.cuda_id, str(self.profile_replay))

        if self.dump:
            logging.info(f"Intermediate benchmark file dumped to {self.dump_path}")
    ...
    def get_tensor_from_file(
        ...
        storage_fn = self.resource_dir + "/nid_" + str(node_id) + "_tid_" + str(tensor_index) + ".dat"
    ...
    def get_tensor_from_storage(
        ...
        if device not in tensor_data[1]:
            if (
                ...
                storage_tensor = torch.rand((tensor_data[0] // elem_bytes), dtype=data_type, device=device)
            else:
                storage_tensor = torch.ones((tensor_data[0] // elem_bytes), dtype=data_type, device=device)
            ...
            for idx, item in enumerate(data_in):
                if is_tensor(node, idx, is_input):
                    if (
                        ...
                        if self.tensor_with_device:
                            item = tuple(item[:5])
                        data_out.append(self.tensor_registry[self.tensors_mapping[(node.id, tuple(item), True)]])
                    ...
                    for t_id in item:
                        if (
                            ...
                            if self.tensor_with_device:
                                ...
                                t_id = tuple(t_id)
                            tensor_list.append(self.tensor_registry[self.tensors_mapping[(node.id, t_id, True)]])
        ...
        try:
            ...
            for idx, item in enumerate(node.outputs):
                if is_tensor(node, idx):
                    ...
                    if self.tensor_with_device:
                        item = tuple(item[:5])
                    outputs.append(self.tensor_registry[self.tensors_mapping[(node.id, tuple(item), True)]])
                elif is_tensor_list(node, idx):
                    self.lookup_cnt += len(item)
                    if self.tensor_with_device:
                        outputs.append(
                            [
                                self.tensor_registry[self.tensors_mapping[(node.id, tuple(t_id[:5]), True)]]
                                for t_id in item
                            ]
                        )
                    else:
                        outputs.append(
                            [self.tensor_registry[self.tensors_mapping[(node.id, tuple(t_id), True)]] for t_id in item]
                        )
                elif item == "<None>" or item == "<Generator>":
                    outputs.append(None)
                elif item == "inf" or item == "-inf":
            ...
            logging.info(f"Outputs error: {e} at node: {node.id}")
    ...
    def run_op(self, node, iter, cnt):  # noqa: C901
        if self.tensor_allocate_mode == TensorAllcationMode.LAZY_ALLOCATE and self.args.device_memory_threshold != 1.0:
        ...
        if isinstance(node, commsArgs):
            ...
            self.commsBench.replaySingle(self.comms_params, node, cnt)
            if self.debug and iter >= self.numWarmupIters:
            ...
            if msg != "":
                logging.info(f"Failed to get input data for node {node.id}: {msg}")
                return False, msg
            ...
            try:
                ...
                if output_count == 0:
                    if node.kernel_backend == "triton":
                        exec(f"func.run(*inputs[:-2], grid={inputs[-2]}, stream={inputs[-1]})")
                    else:
                        func(*inputs)
                else:
                    if output_count == 1:
                    ...
                    # TODO: Simplify this
                    if not tmp:
                        logging.info(f"Not expect that {node.id} has no output.")
                        return
                    for x in tmp:
    ...
    def init_comms(self):
        ...
        logging.info(f"{comms_env_params}, {self.cuda}")
        ...
        self.comms_params = commsParamsHolderBase(comms_args)

        self.commsBench.trace_type = "et"
        self.commsBench.init_backend(bootstrap_info, self.comms_params)
        self.commsBench.initBench(self.comms_params, comms_args)
        self.commsBench.replayInit(self.comms_params)
    ...
    def remove_op_with_runtime_error(self):
        for cnt, node in enumerate(self.sorted_nodes):
            ...
            if success:
                continue
            if msg.find("RuntimeError: CUDA error") != -1 or msg.find("torch.OutOfMemoryError") != -1:
                logging.info(f"Can not keep replaying due to {msg}")
                self.add_skipped_nodes(node, msg)
                break
            ...
            for data_type, t_id, shape in get_output_tensors(node):
                ...
                if data_type == "Tensor(nullptr (uninitialized))":
                    ...
                    dtype, rng = TORCH_DTYPES_RNG[data_type.removeprefix("Tensor(").rstrip(")")]
                    replay_t_id = self.tensors_mapping[(node.id, t_id, False)]
    ...
    def preprocess_graph(self):
        ...
        if self.args.subgraph != "":
            ...
            if not find_subgraph:
                logging.info(f"Cannot find subgraph with name {self.args.subgraph}.")
                exit(1)
        else:
            root = nodes[1]  # 1-base
    ...
    def benchTime(self):
        ...
        if self.args.update_skip_node_file:
            if os.environ.get("CUDA_LAUNCH_BLOCKING", "0") != "1":
                logging.info("Please set CUDA_LAUNCH_BLOCKING=1 to get accurate skip node list.")
                benchmark_result["execution finished"] = False
        ...
        def run_iter(iter):
            ...
            if iter == prev_iter + qps_print_interval:
                logging.info(f"Current QPS: {int(qps_print_interval / ((time.time_ns() - start_ns) / 1000000000))}")
                logging.info(
        ...
        if not self.compute_only:
            ...
            if len(self.profile_step_node_ids) > 1:
                ...
                for node in self.commsBench.comms_trace[: self.commsBench.max_msg_cnt]:
                    if node.id > self.profile_step_node_ids[0] and node.id < self.profile_step_node_ids[1]:
        ...
        if self.profile_replay:
            ...
            with torch.profiler.profile(
                ...
                schedule=torch.profiler.schedule(wait=0, warmup=self.numWarmupIters, active=self.numIters),
                record_shapes=True,
        ...
        if self.profile_memory:
            logging.info("Allocated GPU memory(B):")
            for node in dict(
                sorted(
                ...
                logging.info(f"{node.id}, {self.op_allocated_mem[node]}")
            logging.info("Reserved GPU memory(B):")
            for node in dict(sorted(self.op_reserved_mem.items(), key=lambda item: item[1], reverse=True)[:100]):
                logging.info(f"{node.id}, {self.op_reserved_mem[node]}")
        logging.info("Replay finished")
        logging.info(f"Replay time per iteration: {total_time / self.numIters} ms")
        logging.info(
        ...
        if self.debug:
            logging.info(f"Setup time: {sum(self.setup_time) / 1000000.0}")
            logging.info(f"Execution time: {sum(self.exec_time) / 1000000.0}")

            logging.info(f"Input time: {self.input_total_time / 1000000.0}")
            logging.info(f"Output time: {self.output_total_time / 1000000.0}")
            logging.info(f"Lookup count: {self.lookup_cnt}")
            logging.info(f"Remap tensor list size: {len(self.tensors_mapping)}")
