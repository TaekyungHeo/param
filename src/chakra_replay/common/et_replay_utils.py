    ...
    return (
        (
            ...
            and (
                ...
                or ("fbgemm::" in op.name and "fbgemm::split_embedding_codegen_lookup_" not in op.name)
            )
        )
        or (
            op.name
...
def is_qualified(op):
    return not skip_op(op) and (
        is_backward_aten(op) or is_fbgemm_backward(op) or (is_op(op, strict=True) and not is_backward_parent(op))
    )
